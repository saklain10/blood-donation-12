...
// src/pages/MyDonationRequests.jsx
...
const MyDonationRequests = () => {
  ...
  // Fetch donation requests created by the current user
  const { data: myRequests = [], isLoading, isError, error } = useQuery({
    queryKey: ['myDonationRequests', user?.email, filterStatus], // Query key includes user email and filterStatus
    // Only fetch if user is authenticated and not loading
    enabled: !authLoading && !!user?.email && typeof user.getIdToken === 'function',
    queryFn: async () => {
      let url = `/my-donation-requests`;
      if (filterStatus !== "all") {
        url += `?status=${filterStatus}`; // Backend needs to support this filter
      }
      const res = await axiosSecure.get(url);
      // Frontend filtering for now, if backend doesn't support query param filtering
    ...
    onError: (err) => {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error!',
        text: 'Failed to load your donation requests. Please try again.',
      });
    }
  });

  const pageCount = Math.ceil(myRequests.length / itemsPerPage);
  const offset = currentPage * itemsPerPage;
  const currentItems = myRequests.slice(offset, offset + itemsPerPage);
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (res.data.modifiedCount > 0) {
            ...
            queryClient.invalidateQueries(['myDonationRequests', user?.email, filterStatus]); // Invalidate to refetch
          } else {
  ...
  if (isError) {
    return (
      <div className="text-center py-10">
        <h2 className="text-2xl font-bold text-red-600">Error loading your donation requests.</h2>
        <p className="text-gray-600">Details: {error?.message || 'Unknown error'}</p>
        <p className="text-gray-600 mt-2">Please ensure you are logged in and try again.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      <h1 className="text-3xl font-bold mb-6 text-gray-800">My Blood Donation Requests</h1>
      ...
      {myRequests.length > 0 ? (
        ...
        <div className="mt-8 text-center text-gray-500">
          <p>You have not made any blood donation requests yet.</p>
